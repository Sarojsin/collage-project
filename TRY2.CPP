...
public:
    ...
    void saveDataToCSV()
    {
        ofstream file("cafe_data.csv");//Opens cafe_data.csv in write mode.
        if (!file)
        {
            cout << RED << "Error opening CSV file for saving data." << TERM_RESET << endl;
            return;
        }
        ...
        // Save order data
        file << "\nOrder Data\n";
        file << "OrderID,TableNumber,UserName,ItemsOrdered,TotalAmount\n";
        for (int i = 0; i < orderCount; i++)
        {
        ...
        file.close();
        cout << GREEN << "Data saved successfully to CSV file." <<TERM_RESET << endl;
    }
};
...
void displayCafeArt()
{
    ...
    outtextxy(150, 50, "BY ");        // Display the title
    outtextxy(150, 50, "SAROJ AND RONIT ");        // Display the title
